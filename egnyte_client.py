    ...
    try:
        ...
        if "Developer Over Qps" in e.response.text:
            ...
            try:
                ...
                data = response.json()
                print(f"✅ Folder details retrieved on retry!")
                print(f"   Folder: {data.get('name', 'Unknown')}")
                print(f"   Path: {data.get('path', 'Unknown')}")
                print(f"   ID: {data.get('group_id', 'Unknown')}")
                ...
                print(f"❌ Failed to get folder on retry: {retry_e}")
                print(f"Response: {retry_e.response.text}")
                return None
        else:
    ...
    try:
        ...
        if "Developer Over Qps" in e.response.text:
            ...
            try:
                ...
                print(f"✅ Found {total_items} items (retry):")
                ...
                print(f"❌ Failed to list folder contents on retry: {retry_e}")
                print(f"Response: {retry_e.response.text}")
                return []
        else:
    ...
    try:
        ...
        if "Developer Over Qps" in e.response.text:
            ...
            try:
                ...
                result = response.json()
                print(f"✅ Folder created successfully on retry!")
                print(f"   Name: {result.get('name', folder_name)}")
                print(f"   ID: {result.get('folder_id', 'Unknown')}")
                print(f"   Path: {result.get('path', 'Unknown')}")
                ...
                print(f"❌ Failed to create folder on retry: {retry_e}")
                print(f"Response: {retry_e.response.text}")
                return None
        else:
